...
fn main() {
    ...
    game.run(move |rn| { // 'rn' is an abbreviation of Renoired here.
        rn.grab_cursor(true);
        ...
        player.0 += ((rn.input.pressed(Key::D) as i32 - rn.input.pressed(Key::A) as i32) as f64) * speed * rn.time.delta_time();
        player.1 += ((rn.input.pressed(Key::W) as i32 - rn.input.pressed(Key::S) as i32) as f64) * speed * rn.time.delta_time();
